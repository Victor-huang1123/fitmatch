...
export default function BottomNav() {
  ...
  const visitorNav = [
    { href: "/", label: "首頁", icon: "home", match: (p: string) => p === "/" },
    { href: "/search", label: "搜尋", icon: "search", match: (p: string) => p.startsWith("/search") },
    { href: "/login", label: "登入", icon: "login", match: (p: string) => p.startsWith("/login") || p.startsWith("/register") },
  ];

  const items = user?.role === "merchant" ? merchantNav : user ? studentNav : visitorNav;

  return (
    <nav className="fixed bottom-0 left-0 w-full flex justify-around items-center py-2 px-4 bg-surface shadow-[0_-4px_12px_0_rgba(0,0,0,0.05)] z-50">
      {items.map((item) => {
